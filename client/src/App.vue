...
type Screen = 'jianghu' | 'teams' | 'agents' | 'flows' | 'runs' | 'settings'
...
async function loadAll(): Promise<void> {
  ...
  error.value = ''
  try {
    const [o, orgs, a, t, w, r, c, configs, k] = await Promise.all([
...
async function retryActiveRun(fromTask?: Json): Promise<void> {
  ...
  try {
    ...
    notice.value = fromTask
      ? `已从“${fromTask.node_name}”创建定向重试现场；已完成且不受影响的上游节点及产物会直接保留，原失败现场不变。`
      : `已从失败现场 ${retryInfo?.source_run_id ?? sourceRunId} 创建新的完整重试执行；原现场和失败证据保持不变。`
  } catch (cause) {
