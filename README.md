...
需要 Docker Engine 24+ 和 Docker Compose v2。镜像已经包含 Vue 前端、FastAPI 后端、Python 3.13、Node.js 24、npm 和固定版本 Claude Agent SDK Bridge，不依赖宿主机安装 Agent Runtime。
...
该模式会创建新的 PostgreSQL 领域数据库；不会自动把现有 SQLite 数据覆盖进 PostgreSQL。知识文件、Agent 工作区、Claude Session 状态和加密密钥仍保存在 `.data`。
...
Windows 当前验证环境可使用 `powershell -ExecutionPolicy Bypass -File scripts/start-real-api.ps1` 将 API 作为隐藏后台进程启动。脚本只设置 Claude Code Runtime 与有界超时预算，模型地址、模型名和加密凭据继续从平台数据库读取；stdout/stderr 写入 `.data/logs/`，不会依赖发起命令的临时终端生命周期。
